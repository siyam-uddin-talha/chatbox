            ...
            if (thisUser.friends.length === thisUserFriends.length) {
              const getActiveUser = thisUserFriends.filter((currentUser) => {
                return activeUsers.some(
                  (activeUser) =>
                    activeUser.userEmail === currentUser.friendEmail
                );
              });

              const userWhoisConnectedWithuser = activeUsers.filter(
                (activeUser) => {
                  return getActiveUser.some(
                    (currentUser) =>
                      activeUser.userEmail === currentUser.friendEmail
                  );
                }
                ...
                if (existRoom) {
                  ...
                  existRoom.friends.forEach(
                    async ({ friendEmail, roomId, _id }) => {
                      const userEachFiend = await User.findOne({
                        email: friendEmail,
                      });
                      const { firstName, lastName, photoUrl } = userEachFiend;
                      ...
                      if (
                        existRoom.friends.length === existRoomFriends.length
                      ) {
                        const friendWhoIsActive = existRoomFriends.filter(
                          (user) => user.friendEmail === thisUser.email
                        );
...
const ActiveFriendOverDose = async (io, socket) => {
  try {
    ...
    if (disconnectdUser) {
      const whoIsDisconnected = await User.findOne({
        email: disconnectdUser.userEmail,
      });

      removeAtciveuser(socket.id);

      setTimeout(() => {
        const activeUserWhoisConnectedWithuser = activeUsers.filter(
          (currentUser) => {
            return whoIsDisconnected.friends.some(
              (activeUser) => activeUser.userEmail === currentUser.friendEmail
            );
          }
        );

        activeUserWhoisConnectedWithuser.map((whoisActive) => {
          io.to(whoisActive.socketId).emit(
            "activeUserOver",
            whoIsDisconnected.email
          );
        });
...
const SendMessage = (socket, io) => {
  socket.on("sendMessage", async ({ roomId, sender, text, friendEmail }) => {
    try {
      ...
      const findTheSender = users.find(
        (e) => e.userId === whoSendingThis._id.toString()
      );
      ...
      const response = await Messenger.findOneAndUpdate(
        { _id: roomId },
        {
          $push: {
            room: {
      ...
      const findTheReciver = users.find(
        (e) => e.userId === whoWillReciveData._id.toString()
      );

      if (findTheReciver) {
        //  send the message to the reciver
        io.to(findTheReciver.socketId)
          .to(findTheSender.socketId)
          .emit("getMessage", {
            roomId,
            messages: {
              _id: sorted._id,
...
const NewConversationIsCreated = (io, socket) => {
  socket.on("yourNewConversationCreated", async ({ creator, newFriend }) => {
    const isThisUserActive = activeUsers.find(
      (e) => e.userEmail === newFriend.friendEmail
    );
    ...
    if (isThisUserActive) {
      const connectedUser = await User.findOne({
        email: isThisUserActive.userEmail,
      });
      const { email, firstName, lastName, photoUrl } = await User.findOne({
        _id: creator,
      });

      const { friendEmail, roomId, _id } = connectedUser.friends.find(
        (e) => e.friendEmail === email
      );
