...
const MessengerNavigator = ({ data, socket, loading }) => {
    ...
    return (
        <Box className='messenger_navigator' id='navigator_messenger' >
            <Drawer variant="permanent" className='navigator_drawer __md_screen_navigator height_100' open={true} >
                <>
        ...
        <List className='nav_ul' >
            {data.map((e, index) => (
                ...
                <React.Fragment key={index}  >
                    <Link className='__navigator_link t-d-n LAG_navgiator' style={{ position: 'relative', }} to={`/messages/r/${e.roomId}/?name=${e.friendEmail}`} >
                        <ListItemButton className='__single_user_nav_button' selected={selectedIndex === index} sx={{ borderRadius: ".77rem" }} >
                    ...
                    <Link className='_mobile_navigator__ __navigator_link t-d-n' style={{ position: 'relative', }} to={`/m/messages/r/${e.roomId}/?name=${e.friendEmail}`} >
                        <ListItemButton className='__single_user_nav_button' selected={selectedIndex === index} sx={{ borderRadius: ".77rem" }} >
                            <ListItemIcon sx={{ color: "#15283c" }} >
                                <Avatar alt={`${e.friendName}`} src={e.photoUrl} />
                            </ListItemIcon>
