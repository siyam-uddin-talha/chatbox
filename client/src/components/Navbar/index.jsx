...
export default function AppHeader({ socket, }) {
    ...
    return (
        <>
            ...
            {<NewConvercationDialog user={user} socket={socket} open={openToAddNewConvercation} setOpen={setOpenToAddNewConvercation} email={email} setEmail={setEmail} />}
        </>
    );
}

function NewConvercationDialog({ socket, open, setOpen, email, setEmail, user }) {


    const [CreateNewConvercation, { data, error }] = useMutation(CREATE_NEW_CONVERCATION)
